// BridgeBot.cpp: implementation of the BridgeBot class.
//
//////////////////////////////////////////////////////////////////////
...
BridgeBot::BridgeBot(int inY, int inX, direction_t inDirection, 
	...
	if(!getExploding()) {
		...
		case PIT:
			if((getDirection() == UP) || (getDirection() == DOWN)) {
				getBoard()->setPrimaryTerrainAt(getY(), getX(), NS_BRIDGE);
			}
			else {
				getBoard()->setPrimaryTerrainAt(getY(), getX(), EW_BRIDGE);
			}
			
			setDeployedOnBoard(false);
			getBoard()->removeMoveableAt(getY(), getX(), this);
			break;
		case UPARROW: 
		case R_UPARROW:
			setDirection(UP);
