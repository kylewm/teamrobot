...
void GameUI::on_key_press(const CL_InputEvent &key)
{
	...
	if((key.id == CL_KEY_ENTER) && (key.left_alt || key.right_alt)) {
		if (fullscreen) {
...
void GameUI::on_button_down(const CL_InputEvent &key)
	{
		...
		case CL_MOUSE_LEFT:
			...
			if (!help_on) {
				...
				{
					if ((level_button->get_screen_rect()).is_inside(CL_Point(x, y)) || (level_label->get_screen_rect()).is_inside(CL_Point(x, y))) {
...
void GameUI::draw_controls() {
	...
	font1.draw(618,440, adder + "Generic Bots: " + itoa(myGameDisplay->myBoard->getBotsLeft(), buffer1, 10) 
		+ "/" + itoa(myGameDisplay->myBoard->getNumberOfBots(), buffer2, 10));
...
void GameUI::draw_levelGUI(string levelName) {
	...
	if (!(myGameDisplay->myBoard->getNewBot())) { // if no new bots in this level
		...
		font1.draw(CL_Rect(135, 220, 445, 360), adder + "Generic Bots Available:  " 
			+ itoa(myGameDisplay->myBoard->getNumberOfBots(), buffer, 10) + "\n\n" + myGameDisplay->myBoard->getLevelDescription());
	}
	else {
		font2.draw(CL_Rect(135, 190, 435, 360), levelName);
		font1.draw(CL_Rect(135, 220, 445, 360), adder + "Generic Bots Available:  " 
			+ itoa(myGameDisplay->myBoard->getNumberOfBots(), buffer, 10) + describeNewBot(curEarnedBots) + "\n\n" + myGameDisplay->myBoard->getLevelDescription());
	}
}
